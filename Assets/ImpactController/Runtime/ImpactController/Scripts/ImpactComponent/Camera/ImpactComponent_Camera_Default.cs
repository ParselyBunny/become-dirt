...
namespace JTools
{
    public class ImpactComponent_Camera_Default : ImpactComponent_Camera
    {
        ...
        public override void ComponentLateUpdate(ImpactController player)
        {
            ...
            if (lookTarget == null && !restrainCameraRotating)
            {
                m_camPos = (transform.position + Vector3.up * player.capsuleCollider.height * 0.5f) + (Quaternion.Euler(cameraAngles.x, cameraAngles.y, 0f) * Vector3.Lerp(new Vector3(0.5f, 0.5f, -thirdPersonOrbitDistance * 0.5f), new Vector3(0f, 0f, -thirdPersonOrbitDistance), 1));

                if (Physics.SphereCast(transform.position + Vector3.up * player.capsuleCollider.height * 0.5f, 0.15f, Quaternion.Euler(cameraAngles.x, cameraAngles.y, 0f) * Vector3.Lerp(new Vector3(0.5f, 0.5f, -thirdPersonOrbitDistance * 0.5f), new Vector3(0f, 0f, -thirdPersonOrbitDistance), 1).normalized, out m_hit, Mathf.Lerp(new Vector3(0.5f, 0.5f, -thirdPersonOrbitDistance * 0.5f).magnitude, thirdPersonOrbitDistance, 1), cameraOccluders, QueryTriggerInteraction.Ignore))
                {
